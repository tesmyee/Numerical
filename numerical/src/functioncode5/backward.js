...
function Backward() {
    ...
    function calbackward1() {
        ...
        if (diff === 1) {
            answer = (ans(fn, x) - ans(fn, x - h)) / (h)
            E = diff1(fn, x)
        }
        else if (diff === 2) {
            answer = ((ans(fn, x)) - (2 * ans(fn, x - h)) + ans(fn, x - (2 * h))) / (h ** 2)
            E = diff2(fn, x)
        }
        else if (diff === 3) {
            answer = (ans(fn, x) - 3 * ans(fn, x - h) + 3 * ans(fn, x - (2 * h)) - ans(fn, x - (3 * h))) / Math.pow(h, 3)
            E = diff3(fn, x)
        }
        else if (diff === 4) {
            answer = ((ans(fn, x)) - (4 * ans(fn, x - (h))) + (6 * ans(fn, x - (2 * h))) - (4 * ans(fn, x - (3 * h))) + ans(fn, x - (4 * h))) / (h ** 4)
            E = diff4(fn, x)
        }
        else {
            answer = 0
    ...
    function calbackward2() {
        ...
        var E = 0
        if (diff === 1) {
            answer = ((3 * ans(fn, x)) - (4 * ans(fn, x - h)) + ans(fn, x - (2 * h))) / (2 * h)
            E = diff1(fn, x)
        }
        else if (diff === 2) {
            answer = ((2 * (ans(fn, x))) - (5 * ans(fn, x - h)) + (4 * ans(fn, x - (2 * h))) - (ans(fn, x - (3 * h)))) / (h ** 2)
            E = diff2(fn, x)
        }
        else if (diff === 3) {
            answer = ((5 * ans(fn, x)) - (18 * ans(fn, x - h)) + (24 * ans(fn, x - (2 * h))) - (14 * ans(fn, x - (3 * h))) - (ans(fn, x - (4 * h)))) / (2 * (h ** 3))
            E = diff3(fn, x)
        }
        else if (diff === 4) {
            answer = ((3 * ans(fn, x)) - (14 * ans(fn, x - h)) + (26 * ans(fn, x - (2 * h))) - (24 * ans(fn, x - (3 * h))) + (11 * ans(fn, x - (4 * h))) - (2 * ans(fn, x - (5 * h)))) / (h ** 4)
            E = diff4(fn, x)
        }
        else {
            answer = 0
    ...
    return (
        <Content>
            <Layout style={{ margin: '70px', padding: '10px 10px' }}>
                <h1 style={{ color: 'green' }}>BACKWARD DIFFERENTIAL</h1>
            </Layout>
            <Layout style={{
                margin: '70px',
                padding: '40px 40px',
                ...
                <Row justify="space-around">
                    ...
                    <Col span={8} style={{ textAlign: 'center' }}>
                        <Button type="primary" block id="result" size="large" onClick={calbackward1}> O(h) </Button></Col>
                    <Col span={8}></Col>
                    <Col span={8} style={{ textAlign: 'center' }}>
                        <Button type="primary" block id="result" size="large" onClick={calbackward2}> O(h^2) </Button></Col>
                </Row>
                ...
                <Row type="flex" justify="space-around">
                    <h2 style={{ textAlign: 'center', color: '#E03616' }}> Error : {Es.toFixed(6)}</h2>
                </Row>
            </Layout>

...
const { Column } = Table;
const { parse,random,floor } = require("mathjs");
const { derivative } = require("mathjs");

function Secant() {
    let [x, setx] = useState(0)
    let [x0] = useState(-1)
    let [xtemp, xtemp0] = useState(3)
    ...
    function calsecant() {
        ...
        while (error(x, x0) > ef || i <= 1) {
            x0 = x
            xtemp0 = xtemp
            x = x0 - (ans(fn, x0) * (x0 - xtemp)) / (ans(fn, x0) - ans(fn, xtemp))
            xtemp = x + (Math.abs(xtemp0 - x0))
    ...
    return (
        ...
        <Content>
            <Layout style={{ margin: '70px', padding: '10px 10px' }}>
                <h1 style={{ color: 'green' }}>SECANT</h1>
            </Layout>
            <Layout style={{
                margin: '70px',
                padding: '40px 40px',
                ...
                <Row justify="space-around">
                    ...
                    <Col span={8} style={{ textAlign: 'center' }}>
                        <Button type="primary" block id="result" size="large" onClick={calsecant}> Submit </Button></Col>
                </Row>
